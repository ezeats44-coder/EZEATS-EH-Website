...
export function recipeDatabase(){
 if(!process.env.RECIPE_DATABASE_URL)return null;
 database??=neonDatabase(new Pool({connectionString:process.env.RECIPE_DATABASE_URL,max:3,connectionTimeoutMillis:5000,query_timeout:10000}));return database;
}
export function productionRecipeStore(){
 ...
 store??=createRecipeStore(recipeDatabase());
 return store;
}

...
export function createOwnerRecipesHandler({store,authOptions,enabled=()=>process.env.OWNER_RECIPES_ENABLED==='true'}={}){return async(req,res)=>{
 ...
 try{
  ...
  const actor={authorized:true,userId:who.userId};await db.rateLimit(actor);
  ...
  if(req.method==='GET'){
   ...
   if(req.query?.id){if(Object.keys(req.query).length!==1)throw new WorkspaceError('Do not combine recipe ID and list filters.');return res.status(200).json({record:await db.get(identity(req.query.id),actor)});}
  ...
  keys(b,['action','id','expectedLock','content','notes']);identity(b.id);
  ...
  return res.status(b.action==='create'?201:200).json({record:await db.mutate(b,actor)});
